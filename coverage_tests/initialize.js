...
module.exports = function (tracker, test) {
  ...
  const eyes = {
    ...
    open({appName, testName, viewportSize}) {
	  let save = false
	  if ((test.name === 'check region by selector after manual scroll with scroll stitching') || (test.name === 'check frame fully with css stitching')) save = true
